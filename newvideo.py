...
class SSearchHandler(tornado.web.RequestHandler):
    def get(self):
        c = redis.Redis(host='192.168.0.32', port=6379, db=4)
        ...
        for id in ids:
            ...
            if _item.has_key("director"):
                ...
                try:
                    _item['director'] = _item['director'][0]
                except:
                    _item['director'] = '刘志达'
            if _item.has_key("actor"):
                _item['actor'] = eval(_item['actor'])
                try:
                    _item['actor'] = _item['actor'][0]
                except:
                    _item['actor'] = "无"
            if _item.has_key("cate"):
                _item['cate'] = eval(_item['cate'])
                try:
                    _item['cate'] = _item['cate'][0]
                except:
                    _item['cate'] = "无"
            if _item.has_key("urls"):
                del _item['urls']
            if _item.has_key("down"):
                del _item["down"]
                #_item['down'] = json.loads(_item['down'])
                #for down in _item['down'].values():
                #    if "itc.cn" in down:
                #        _item['down'] = down
                #    else:
                #        continue
...
class MiniHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        newtv = []
        _items = c.zrange("new:tv",0,25,desc=False)
        for i in _items:
            id = i
            if not c.exists("id:%s"%id):
                continue
        ...
        toptv = []
        _items = c.zrange("week:tv",0,25,desc=False)
        x = 0
        for i in _items:
            id = i
            if not c.exists("id:%s"%id):
        ...
        newmovie = []
        _items = c.zrange("new:movie",0,25,desc=False)
        for i in _items:
            id = i
            if not c.exists("id:%s"%id):
                continue
        ...
        topmovie = []
        _items = c.zrange("week:movie",0,25,desc=False)
        x = 0
        for i in _items:
            id = i
            if not c.exists("id:%s"%id):
...
class TinyHandler(tornado.web.RequestHandler):
    def get(self):
        c = redis.Redis(host='192.168.0.32', port=6379, db=4)
        ...
        for i in _items:
            ...
            _item['score'] = c.hget("id:%s"%id,"score")
            topmovie.append(_item)
        self.render("miniindex.html",newtv=newtv,toptv=toptv,topmovie=topmovie,newmovie=newmovie)
...
def main():
    ...
    application = tornado.web.Application([
        ...
        (r"/api/sign",              GetsignHandler),
        (r"/api/set/(.*)",          SeturlHandler),
        (r"/api/s",                 SearchHandler),
        (r"/api/ss",                SSearchHandler),
