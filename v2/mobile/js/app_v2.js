...
(function(window, document, undefined){
	...
	if(typeof window.videoTaskPlugin == 'undefined'){
		window.videoTaskPlugin = {
			...
			getStatus : function(id){
				...
				return OTS_NONE;//OTS_FINISH_SUCCESS;
			},
			play : function(id){
				//console.log('getStatus:'+id);
			},
	...
	UI.history = {
		...
		//only for log
		last : function(){
			var len = UI.history.list.length;
			if(len >= 2){
				return UI.history.list[len - 2].type;
			}else{
				return '';
			}
		},
	...
	//get content url
	window.downUrl = function(id, params){
		var paramStr = '';
		for(var i in params){
			if(params[i]){
		...
		return 'http://video.wandoujia.com/api/down/'+id+(paramStr ? '?'+paramStr : '');
	}
	...
	window.runTask = function(o, add){
		...
		var obj = $(o), id = obj.data('id'), size = obj.data('size') || '', status = videoTaskPlugin.getStatus(id);
		if(status == OTS_FINISH_SUCCESS){
			videoTaskPlugin.play(id);
			//
			gLog('TaskPlay', currentShow +(UI.history.last() ? '_'+UI.history.last() : ''), id);
		}else if(status == OTS_NONE){
			videoTaskPlugin.addTask(id, (obj.data('tv') ? obj.data('tv') +'-' : '')+obj.data('title'), '', downUrl(id, {size:size}));
			if(obj.data('style')){
				obj.addClass(obj.data('style'));
			}
			//
			gLog('TaskCreate', currentShow+(UI.history.last() ? '_'+UI.history.last() : ''), id);
		}else{
			...
			//
			gLog('TaskRemove', currentShow+(UI.history.last() ? '_'+UI.history.last() : ''), id);
		}
	...
	window.loadGroup = function (o, page, callback, byBack){
		...
		var lastTab = $(".group").find('.current'), 
			...
			cacheName = thisTab.data('id') ? thisTab.data('id') + type : type,
		...
		pageCaches[cacheName] = pageCaches[cacheName] || [];
		if(currentShow == 'singer-detail')pageCaches[cacheName][page] = undefined;
		if(typeof pageCaches[cacheName][page] == 'undefined'){
					...
					switch(type){
						...
						default:
							if(typeof data.items != 'undefined'){
								...
								pageCaches[cacheName][page] = data;
					...
					if(type == 'movie'){
						...
						$packObj.bind('click', function(){
							...
							while(idx--){
								if(videoTaskPlugin.getStatus(data.show[idx].id) == OTS_NONE){
									allnum++;
									var record = data.show[idx], size = record['size'] || '';
									//content_id$$content_title$$content_icon$$content_url
									if(allnum > 1){
										contentStr = '##' + contentStr;
									}
									
									contentStr = record.id + '$$'+ data.title + '-'+ record.title + '$$' + imgUrl(id) + '$$' + downUrl(record.id, {size:size}) + contentStr;
								}
							}
							//取消
							if(allnum > 0){
	...
	//load default tab
	var defaultTab = window.pageIndex || location.hash.substr(1);
